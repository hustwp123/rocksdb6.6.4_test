...
Status BlockBasedTable::IndexReaderCommon::ReadIndexBlock(
  ...
  perf_context.my_read_index_block_nanos+=Env::Default()->NowNanos()-now;
  fprintf(stderr,"read_index_block_nanos:%lu\n",perf_context.my_read_index_block_nanos);
  return s;
}
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (!may_match) {
    ...
    bool done = false;
    uint64_t now_index=Env::Default()->NowNanos();
    iiter->Seek(key);
    perf_context.my_index_search_nanos+=Env::Default()->NowNanos()-now_index;
    ...
    for (; iiter->Valid() && !done; iiter->Next()) {
      ...
      uint64_t now_data=Env::Default()->NowNanos();
      ...
      perf_context.my_data_search_nanos+=Env::Default()->NowNanos()-now_data;
